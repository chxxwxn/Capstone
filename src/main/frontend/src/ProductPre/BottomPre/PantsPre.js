...
const Preview = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("http://localhost:8090/products/bottom/pants");
        ...
        <div className={styles.categoryTop}>
          ...
          {categories.map((category, index) => (
              <div
                ...
                style={{
                  cursor: "pointer",
                  color: category.name === "Pants" ? "black" : "rgba(0, 0, 0, 0.5)", 
                  borderBottom: category.name === "Pants" ? "2px solid black" : "none"
                }}
        ...
        <div className={styles.productGrid}>
          {paginatedProducts.map((product) => (
            <div key={product.id} className={styles.product}>
              <a href={`/bottom/pants/${product.id}`}>
