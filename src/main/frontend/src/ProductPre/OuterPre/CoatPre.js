...
const Preview = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("http://localhost:8090/products/outer/coat");
        ...
        <div className={styles.categoryTop}>
          ...
          {categories.map((category, index) => (
              <div
                ...
                style={{
                  cursor: "pointer",
                  color: category.name === "Coat" ? "black" : "rgba(0, 0, 0, 0.5)", 
                  borderBottom: category.name === "Coat" ? "2px solid black" : "none"
                }}
