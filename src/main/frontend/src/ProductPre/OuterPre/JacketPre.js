...
const Preview = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("http://localhost:8090/products/outer/jacket");
        ...
        <div className={styles.categoryTop}>
          ...
          {categories.map((category, index) => (
              <div
                ...
                style={{
                  cursor: "pointer",
                  color: category.name === "Jacket" ? "black" : "rgba(0, 0, 0, 0.5)", 
                  borderBottom: category.name === "Jacket" ? "2px solid black" : "none"
                }}
        ...
        <div className={styles.productGrid}>
          {paginatedProducts.map((product) => (
            <div key={product.id} className={styles.product}>
              <a href={`/outer/jacket/${product.id}`}>
