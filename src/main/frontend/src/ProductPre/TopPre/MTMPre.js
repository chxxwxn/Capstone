...
const Preview = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("http://localhost:8090/products/top/MTM");
        ...
        <div className={styles.categoryTop}>
          ...
          {categories.map((category, index) => (
              <div
                ...
                style={{
                  cursor: "pointer",
                  color: category.name === "MTM" ? "black" : "rgba(0, 0, 0, 0.5)", 
                  borderBottom: category.name === "MTM" ? "2px solid black" : "none"
                }}
