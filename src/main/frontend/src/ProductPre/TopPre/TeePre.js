...
const Preview = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("http://localhost:8090/product/top/tee");
        ...
        <div className={styles.categoryTop}>
          ...
          {categories.map((category, index) => (
              <div
                ...
                style={{
                  cursor: "pointer",
                  color: category.name === "Tee" ? "black" : "rgba(0, 0, 0, 0.5)", 
                  borderBottom: category.name === "Tee" ? "2px solid black" : "none"
                }}
